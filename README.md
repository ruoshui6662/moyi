...
**Chrome 插件**
- 核心管线：段落候选发现、视口优先分批（批 8 并发 3，单批输入 6000 字符预算）、流式逐段渲染（增量只写文字不重建样式）、视口下 1400px 窗口预取 + 近窗口插队、长文滚动节流补扫（突破单次 100 段上限）、关页尽力刷写缓存
- 服务商：OpenAI 兼容 ×5（OpenAI / DeepSeek / Kimi / MiniMax / 智谱 GLM）+ 自定义服务商增删 + DeepL 官方 API（免费/专业套餐）+ 腾讯翻译（TMT，SecretId/SecretKey）+ 微软翻译（Edge 免密钥端点）+ 谷歌翻译（translate-pa 免密钥端点）+ **Ollama（仅 Chrome 插件，本机默认免密钥）**
- 设置页：凭据管理与校验、获取模型列表、测试连接、关闭推理模式、提示词风格/自定义提示词、译文样式六件套、快捷键录制、恢复出厂
- 渲染：六种译文样式（五种双语 + 直接替换）、字号/行距/字距/字体独立调节、暗色背景自适应提亮、替换模式（隐藏原文）
- 段落级译文缓存（7 天 TTL / 5000 条 LRU）
- 悬浮按钮：拖拽、边缘吸附胶囊、悬停展开、长按手势钩子、大小（26–48px）与透明度可调
- **视频字幕双语翻译**：YouTube（json3 轨道）与 X/Twitter（HLS + WebVTT）播放器内叠加双语字幕；规则断句（两阶段装配 + 显示规划）+ 可选 AI 断句（OpenAI 兼容服务商，失败自动回退规则）；播放进度驱动按需预取、断句单元与译文双层缓存、Shadow DOM 覆层与播放器控制条集成
...
- TypeScript strict 全量类型检查通过；**401 项 Vitest 单测（37 个文件）全部通过**（覆盖共享核心、双端适配层、字幕子系统、边界清洗）
- 双发行版均可一键构建；产物已实测加载
...
- **多种服务商**：
  ...
  - Ollama（**仅 Chrome 插件**，OpenAI 兼容本机服务；默认 `http://localhost:11434/v1`，填写模型即可，无需 API Key）。
- **五种双语样式 + 直接替换**：朱砂界线 / 黛青界线 / 竹青下划 / 月白高亮 / 无标记，以及隐藏原文的「直接替换」模式（共六种版式预设）。
- **视频字幕双语翻译（仅插件）**：YouTube 与 X/Twitter 视频播放器内叠加双语字幕；规则断句 + 可选 AI 断句（OpenAI 兼容服务商）；可在设置 → 字幕中开关。
...
| Ollama（仅 Chrome 插件） | 启动本机 Ollama 后填写模型（例如 `qwen3:8b`）；默认接口 `http://localhost:11434/v1`，无需 API Key。首次使用先执行 `ollama pull <模型>`；若浏览器请求被 403，需设置 `OLLAMA_ORIGINS` 放行扩展来源 |
