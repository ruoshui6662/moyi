...
- TypeScript strict 全量类型检查通过；**374 项 Vitest 单测（36 个文件）全部通过**（覆盖共享核心、双端适配层、字幕子系统、边界清洗）
- 双发行版均可一键构建；产物已实测加载
...
9. 除腾讯（限流退避 2 次）外，其余服务商无自动重试，失败即报错待用户重触发；
...
11. Ollama 本机服务商接入已完成开发与单测，但该批改动（8 改 3 增）**尚未提交 git**。
